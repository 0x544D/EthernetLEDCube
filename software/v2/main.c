...
int main(int argc, char *argv[])
{
    ...
    pthread_mutex_init (&ledCubeMutex, NULL);
    pthread_mutex_init (&etherFrameMutex, NULL);

    showLedCube = &ledCube0;
	drawLedCube = &ledCube1;

	sendBuf = etherFrameBuff0;
	sendMsg = etherFrameBuff1;
  ...
  for (int y = 0; y < CUBE_DIM; y++) {
    ...
    for (uint8_t x = 0; x < CUBE_DIM; x++) {
        for (uint8_t z = 0; z < CUBE_DIM; z++) {
           ...
           double r = (*showLedCube)[x][y][z].r / 255.0;
           double g = (*showLedCube)[x][y][z].g / 255.0;
           double b = (*showLedCube)[x][y][z].b / 255.0;
