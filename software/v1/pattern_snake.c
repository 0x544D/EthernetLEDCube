...
void snake_loop() {
	...
	ledCube[posX][posY][posZ] = hsvToRGB(color);
		...
		for (uint8_t x = 0; x < CUBE_DIM; x++) {
			for (uint8_t y = 0; y < CUBE_DIM; y++) {
				for (uint8_t z = 0; z < CUBE_DIM; z++) {
					...
					if (ledCube[x][y][z].r > 3)
						ledCube[x][y][z].r -= 4;
					else if (ledCube[x][y][z].r > 0)
						ledCube[x][y][z].r -= 1;

					if (ledCube[x][y][z].g > 3)
						ledCube[x][y][z].g -= 4;
					else if (ledCube[x][y][z].g > 0)
						ledCube[x][y][z].g -= 1;

					if (ledCube[x][y][z].b > 3)
						ledCube[x][y][z].b -= 4;
					else if (ledCube[x][y][z].b > 0)
						ledCube[x][y][z].b -= 1;
				}
			}
		}
...
uint8_t checkPossibleMoveDirs(uint16_t posX, uint16_t posY, uint16_t posZ){
	...
	if (posX+1 < CUBE_DIM) {
		if (ledCube[posX+1][posY][posZ].r == 0 && ledCube[posX+1][posY][posZ].g == 0 && ledCube[posX+1][posY][posZ].b == 0) {
			moveDirs |= (1 << DIRX);
		}
	...
	if (posY+1 < CUBE_DIM) {
		if (ledCube[posX][posY+1][posZ].r == 0 && ledCube[posX][posY+1][posZ].g == 0 && ledCube[posX][posY+1][posZ].b == 0) {
			moveDirs |= (1 << DIRY);
		}
	...
	if (posZ+1 < CUBE_DIM) {
		if (ledCube[posX][posY][posZ+1].r == 0 && ledCube[posX][posY][posZ+1].g == 0 && ledCube[posX][posY][posZ+1].b == 0) {
			moveDirs |= (1 << DIRZ);
		}
